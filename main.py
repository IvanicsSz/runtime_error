    ...
    for i in range(15):
        for j in range(13):
            if i == 0 or i == 14 or j == 0 or j == 12 or (i % 2 == 0 and j % 2 == 0):
                gamefield[i][j] = Wall(x=i*78, y=j*78, image=stone, fix=True)
    gamefield[1][1] = Player(x=1*78, y=1*78, image=player_1)
    gamefield[13][11] = Player(x=13*78, y=11*78, image=player_2)
        ...
        for h, row in enumerate(column):
            if not (row != 0 or (f == 2 and h == 1) or (f == 1 and h == 2) or (f == 12 and h == 11) or (f == 13 and h == 10)):
                if random.choice([0, 1]):
                    gamefield[f][h] = Wall(x=f*78, y=h*78, image=brick, fix=False)
